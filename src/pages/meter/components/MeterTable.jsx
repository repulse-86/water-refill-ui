...
export default function MeterTable({ readings, isLoading, viewMode = 'active', onEdit, onDelete, onRestore, onPermanentDelete }) {
  const baseColumns = [
    { accessorKey: 'reading_date', header: 'Date', render: (value) => formatDate(value) },
    { accessorKey: 'meter_value', header: 'Meter Reading', render: (value) => `${formatValue(value)} gal` },
    { accessorKey: 'expected_volume', header: 'Expected', render: (value) => `${formatValue(value)} gal` },
    { accessorKey: 'actual_throughput', header: 'Throughput', render: (value) => `${formatValue(value)} gal` },
    {
      ...
      render: (value, row) => {
        ...
        return <span className={row.flagged ? 'font-medium text-red-600' : 'text-slate-700'}>{formatValue(value)} gal{pct}</span>;
      },
    },
    {
      accessorKey: 'flagged',
